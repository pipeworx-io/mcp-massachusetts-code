 ...
 *
 * Massachusetts cites as "chapter, section" — c. 265, § 1 — but its URLs need
 * the PART and TITLE the chapter belongs to, which nobody knows from the
 * citation:
 *
 *   /Laws/GeneralLaws/PartIV/TitleI/Chapter265/Section1
 *                     ^^^^^^ ^^^^^^ neither appears in "c. 265, s. 1"
 *
 * Rather than ship a 282-chapter lookup table that would drift, this resolves
 * the chapter through the site's own chapter URL, which redirects to the
 * canonical Part/Title path. One extra request buys correctness that a baked
 * table cannot keep.
 *
 * A nonexistent section 404s honestly.
 */
...
async function pwFetch(url: string | URL, init?: RequestInit): Promise<Response> {
  return fetchWithTimeout(url, init ?? {}, 'Massachusetts General Laws');
}

const BASE = 'https://malegislature.gov/Laws/GeneralLaws';
const UA = 'pipeworx-mcp-massachusetts-code/1.0 (+https://pipeworx.io)';
...
const tools: McpToolExport['tools'] = [{
  ...
  inputSchema: {
    type: 'object',
    properties: {
      chapter: { type: 'string', description: 'Chapter number, e.g. "265" (crimes against the person) or "90" (motor vehicles).' },
      section: { type: 'string', description: 'Section within the chapter, e.g. "1" or "24".' },
    },
...
async function callTool(name: string, args: Record<string, unknown>): Promise<unknown> {
  if (name !== 'ma_general_law') throw new Error(`Unknown tool: ${name}`);
  const chapter = String(args.chapter ?? '').trim().replace(/[^0-9A-Za-z]/g, '');
  const section = String(args.section ?? '').trim().replace(/[^0-9A-Za-z.]/g, '');
  if (!chapter || !section) {
    return { found: false, reason: 'missing_argument',
      message: 'Both chapter and section are required, e.g. {chapter: "265", section: "1"}.' };
  }

  // Resolve Part/Title by following the chapter URL rather than hard-coding a
  // 282-row table. The redirect target carries the canonical path.
  const chapRes = await pwFetch(`${BASE}/Chapter${encodeURIComponent(chapter)}`, {
    headers: { 'User-Agent': UA }, redirect: 'follow',
  });
  const pt = chapRes.url.match(/GeneralLaws\/(Part[IVX]+)\/(Title[IVX]+)\//);
  if (!pt) {
    return { found: false, reason: 'chapter_not_found', chapter,
      message: `Could not locate chapter ${chapter} in the Massachusetts General Laws.`,
      hint: 'Chapters run 1-282. Crimes against the person is 265; motor vehicles is 90.' };
  }

  const url = `${BASE}/${pt[1]}/${pt[2]}/Chapter${chapter}/Section${section}`;
  const res = await pwFetch(url, { headers: { 'User-Agent': UA } });
  if (res.status === 404) {
    return { found: false, reason: 'section_not_found', chapter, section,
      message: `No section ${section} in Massachusetts General Laws chapter ${chapter} (HTTP 404).`,
      hint: 'Section numbers are not contiguous — repealed sections are normal.' };
  }
  if (!res.ok) throw new Error(`Massachusetts Legislature returned HTTP ${res.status}`);
  const html = await res.text();

  // The statute follows an h2.genLawHeading; converting the whole page puts the
  // site's breadcrumb ("General Law - Part IV, Title I, ...") where the law
  // should be. Take the heading from the h2 and the text from after it.
  const hm = html.match(/class="[^"]*genLawHeading[^"]*"[^>]*>([\s\S]*?)<\/h2>/i);
  const heading = hm ? statuteText(hm[1]) : null;
  const after = hm ? html.slice((hm.index ?? 0) + hm[0].length) : html;
  const text = statuteText(after);
  return {
    citation: `M.G.L. c. ${chapter}, § ${section}`,
    chapter, section, part: pt[1], title: pt[2],
    heading,
    text, text_chars: text.length, url,
    source: 'Massachusetts Legislature (malegislature.gov), keyless',
    attribution: 'Massachusetts statutes are public record.',
  };
}
